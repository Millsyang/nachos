...
#define STACK_FENCEPOST 0xdeadbeef	// this is put at the top of the
					...
					// stack overflows
//----------------------------------------------------------------------
// allocate ThreadID for each just created thread,if now id remained,
// return -1
//----------------------------------------------------------------------
...
{
	//limit num of thrad up to 128
	int flag = allocateThreadID();
	if (flag == -1)	printf("Up to threadnum limit,stop creating thread\n");
	//How to prevent stoping and then just give up creating new thread?
	ASSERT(flag != -1);
	if(p < 0)
	{
		priority = 0;
...
{ 
	Thread* t = (Thread*)arg;
	t->Print();
	printf("threadid %d status:%s\n",t->getThreadID(),t->getStatus());
}
