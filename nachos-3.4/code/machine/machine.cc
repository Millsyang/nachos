...
Machine::freeMem(){
    int shift;
    for (shift = 0;shift < NumPhysPages;shift++){
        if (bitmap >> shift & 0x1) { // found used bit
            bitmap ^= 0x1 << shift; // set the bit to empty
            //In function AddressSpaceControlHandler
            //current thread space will be released
            //so we donot need to set pagetable invalid here
            DEBUG('M', "Release physical page frame: %d\n", shift);
        }
    }
}